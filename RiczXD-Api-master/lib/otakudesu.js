...
const res = await axios.get(baseUrl+`https://otakudesu.moe/ongoing-anime`)
...
const res_ = await axios.get(baseUrl+`https://otakudesu.moe/?s=${search}&post_type=anime`)
...
const ress_ = await axios.get(baseUrl+`${link_}`)
...
return  { status: res_.status, result: hasil }
}
