    ...
    return new Promise((resolve, reject) => {
        fetch('https://jagokata.com/kata-bijak/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
            ...
            .then(res => {
                ...
                resolve({ creator: '#𝑪𝒖𝒑𝒛', status: true, data })
            }).catch(reject)
    })
}
...
function facebook2(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data.links.lenght == 0) return resolve({ creator: '#𝑪𝒖𝒑𝒛', status: false })
                resolve({ creator: '#𝑪𝒖𝒑𝒛', status: true, data: data.links })
            })
    })
}

module.exports = { facebook2, quotes, igdl, igstory2, tiktok, twitter2, joox, pindl }
